...
export class AuthResourceService extends ResourceBase {
  ...
  public register(model: RegistrationInfo): Observable<Account | null> {
    return this.post('/auth/register', model.toDto()).pipe(
      map((result: any) => {
        if (result) {
  ...
  public login(model: LoginInfo): Observable<Credential | null> {
    return this.post('/auth/login', model.toDto()).pipe(
      map((result: any) => {
        if (result) {
